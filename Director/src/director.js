...
class Director{
    ...
    _wireSocketServerEvents(){
        this._socketServer.on(this._socketServer.events.SOCKET_ON_NEW_STATUS_CONNECTION,this._newSerialConnection);
    }
    ...
    _robotDataReceived(data,robot){
        ...
        if(!robot.endpoint){
            robot.endpoint = this._webServer.createWSEndpoint(`/robots/${robot.status.name}`, 
            (ws,req)=>{
                console.log(`\tSomeone is listenting to ${robot.status.name}`.yellow.bold);
                robot.ws = ws;
            });
        }
        if(robot.ws)
